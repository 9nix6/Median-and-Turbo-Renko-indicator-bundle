...
//--- get Fast EMA buffer
   ...
   ExponentialMAOnBuffer(rates_total,_prev_calculated,0,InpFastEMA,medianRenkoIndicator.Close,ExtFastMaBuffer);
//--- get SlowSMA buffer
   if(IsStopped()) return(0); //Checking for stop flag
   ExponentialMAOnBuffer(rates_total,_prev_calculated,0,InpSlowEMA,medianRenkoIndicator.Close,ExtSlowMaBuffer);
//---
   int limit;
