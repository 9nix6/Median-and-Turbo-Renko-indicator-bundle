...
class MedianRenkoIndicator
{
   ...
   public:
      ...
      // TODO - BufferShiftRight(double &buffer[]);
...
/*
int MedianRenkoIndicator::GetOLHCForIndicatorCalc(double &o[],double &l[],double &h[],double &c[], int start, int count)
{
   double temp[];
   ...
   int _count = CopyBuffer(medianRenkoHandle,RENKO_OPEN,start,count,temp);
   if(_count == -1)
      return _count;
   ...
   if(_count < count)
   {
      ...
      if(CopyBuffer(medianRenkoHandle,RENKO_LOW,start,_count,temp) == -1)
         return -1;
      ArrayCopy(l,temp,(count-_count),0);
         
      if(CopyBuffer(medianRenkoHandle,RENKO_HIGH,start,_count,temp) == -1)
         return -1;
      ArrayCopy(h,temp,(count-_count),0);

      if(CopyBuffer(medianRenkoHandle,RENKO_CLOSE,start,_count,temp) == -1)
         return -1;
